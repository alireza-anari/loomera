...
export default function initEditProfile() {
        ...
        reader.onload = (e) => {
          if (profileImagePreview) {
            if (profileImagePreview.tagName === "IMG") {
              ...
              img.className =
                "w-full h-full rounded-full object-cover border-4 border-white shadow-md";
  ...
  if (editForm) {
    editForm.addEventListener("submit", (e) => {
      ...
      const birthMonth = editForm.querySelector(
        'select[name="birth_month"]',
      ).value;
      const birthYear = editForm.querySelector(
      ...
      if (anyBirthFieldFilled && !allBirthFieldsFilled) {
        e.preventDefault();
        alert(
          "لطفاً تمام فیلدهای تاریخ تولد را پر کنید یا همه را خالی بگذارید.",
        );
      }
      ...
      if (
        birthYear &&
        (parseInt(birthYear) < 1300 || parseInt(birthYear) > 1410)
      ) {
        e.preventDefault();
