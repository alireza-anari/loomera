...
class ApiV1BookingDraftValidationTests(Stage1DomainFactoryMixin, TestCase):
    ...
    def test_draft_validate_accepts_hidden_resume_and_reports_unavailable_slot(self):
        customer, salon, service, stylist, target_date = self._setup_available_slot()
        self.client.force_login(customer.user)

        hidden_stylist = self.make_stylist(
            public_visibility=Stylist.PublicVisibility.HIDDEN,
            resume_headline="PRIVATE_HIDDEN_VALIDATE_HEADLINE",
            resume_summary="PRIVATE_HIDDEN_VALIDATE_SUMMARY",
        )
        ...
        body = response.content.decode("utf-8")
        self.assertNotIn("PRIVATE_HIDDEN_VALIDATE_HEADLINE", body)
        self.assertNotIn("PRIVATE_HIDDEN_VALIDATE_SUMMARY", body)
