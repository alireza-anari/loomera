...
## Availability preview follow-up

Loomi now previews real booking availability for scoped salon/stylist conversations
without creating an Order or reserving a slot. Availability reads reuse
`apps.orders.booking_utils.get_available_slots_for_service`; the existing booking
flow remains the only write/confirmation path.

- `امروز`, `فردا` and `پس فردا` use an exact one-day availability window.
- Generic availability questions and `این هفته` search up to seven days ahead.
- When a target exposes several bookable services and the question does not name
  one, Loomi asks the user to select a service with provider callbacks. Existing
  menu/action callbacks still have higher dispatcher priority.
- Salon previews only use active, publicly visible specialists who can enter the
  existing public booking flow. Specialist previews only use visible active salon
  relationships supported by that booking flow.
- Up to five real slots are displayed. Each slot button contains a short-lived
  signed `service_stylist_time` quick-booking payload with salon, service,
  specialist, date and time. Opening it transfers the selection into the existing
  website booking session and reservation preview, where availability is
  revalidated before any confirmation.
- No `Order`, `OrderDetail`, payment, hold, slot reservation or new availability
  cache is created by Loomi.

### Availability manual QA

1. Open a salon Loomi deep link with at least two active services. Ask `برای فردا
   وقت دارید؟`; choose a service and verify the callback returns real slots.
2. Ask `فردا برای رنگ مو چه ساعتی خالیه؟`; verify the service is inferred and the
   displayed slots match the website booking engine for the same salon/service.
3. Tap one slot. Verify Loomera opens with the same salon, service, specialist,
   date and time already selected and still requires the normal final booking
   confirmation/login flow.
4. Repeat through a specialist deep link. If the specialist works at multiple
   bookable salons, verify each displayed slot names the correct salon.
5. Hide/deactivate the specialist or remove the service from public booking and
   repeat; no signed slot link for the hidden/inactive target may be returned.
6. Ask for a full day with no availability. Loomi must say that no time was found
   and offer the normal booking route for other dates; it must not invent a time.
7. Open the bot without a Loomi deep link and ask `برای فردا وقت دارید؟`; it must
   still request salon/specialist selection and must not query availability.
